...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --speed-limit 1 --speed-time 300 -o ${xui_folder}-linux-$(arch).tar.gz https://gh.ikl.ink/https://github.com/MHSanaei/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://gh.ikl.ink/https://github.com/MHSanaei/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -fLRo "${xui_script_temp}" https://gh.ikl.ink/https://raw.githubusercontent.com/MHSanaei/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://gh.ikl.ink/https://raw.githubusercontent.com/MHSanaei/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://gh.ikl.ink/https://raw.githubusercontent.com/MHSanaei/3x-ui/main/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://gh.ikl.ink/https://raw.githubusercontent.com/MHSanaei/3x-ui/main/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://gh.ikl.ink/https://raw.githubusercontent.com/MHSanaei/3x-ui/main/x-ui.service.rhel"
                    ;;
            esac
